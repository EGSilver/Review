...
void Player::death(Creature* _lastHitCreature)
{
	...
	if (skillLoss) {
		...
		if (_lastHitCreature) {
			if (Player* player = _lastHitCreature->getPlayer()) {
				int32_t v = 0;
				getStorageValue(300000, v);
				if (v > 0) {
					player->getStorageValue(300000, v);
					if (v > 0) {
						player->getStorageValue(300003, v);
						
						if (v < 0) {
							v = 0;
						}
						
						player->addStorageValue(300003, v + 1);
						std::ostringstream ss;
						ss << "[PK EVENT] You now have " << v + 1 << " kills!";
						player->sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
						sendTextMessage(MESSAGE_EVENT_ADVANCE, "[PK EVENT] You have been killed during the PK Event, please relog.");
					}
				}
			}
		}
...
bool Player::onKilledCreature(Creature* target, bool lastHit/* = true*/)
{
	...
	if (Player* targetPlayer = target->getPlayer()) {
		int32_t value = -1;
		getStorageValue(300000, value);
		if (value > -1) {
			targetPlayer->getStorageValue(300000, value);
			if (value > -1) {
				return false;
			}
		}
